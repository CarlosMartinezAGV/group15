...
function readCookie() {
  ...
  if (userId < 0) {
    ...
    document.getElementById('userName').innerHTML =
      "   Welcome! <br /><i class='fa fa-user' aria-hidden='true'></i> " +
      firstName +
      ' ' +
      lastName +
      ''
  ...
  if (firstName == '') {
    ...
    try {
      xhr.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
          document.getElementById('registerResult').innerHTML =
            'User has been registered. Please login now'
        }
      }
      xhr.send(jsonPayload)
    } catch (err) {
    ...
    xhr.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        if (xhr.responseText == 'No Records Found') {
          ...
          document.getElementById('contactSearchResult').innerHTML =
            'Search Results: ' + jsonObject.results.length
          ...
          for (let i = 0; i < jsonObject.results.length; i++) {
            ...
            buttonList +=
              "<button class='editbutton' onclick = 'test(" +
              ...
              '</button>'
            buttondeleteList +=
              "<button class='deletebutton' onclick = 'remove(" +
              jsonObject.results[i].ID +
              ");'>" +
